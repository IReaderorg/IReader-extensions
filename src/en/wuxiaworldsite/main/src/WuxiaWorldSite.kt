package tachiyomix.wuxiaworldsite

import ParsedHttpSource
...
@Extension
abstract class WuxiaWorld(private val deps: Dependencies,private val okHttpClient: OkHttpClient) : ParsedHttpSource(deps) {
  ...
  override suspend fun getChapterList(book: MangaInfo): List<ChapterInfo> {
    return kotlin.runCatching {
      return@runCatching withContext(Dispatchers.IO) {
        var chapters =
          chaptersParse(client.post<String>(requestBuilder(book.key + "ajax/chapters/")).parseHtml())
        if (chapters.isEmpty()) {
