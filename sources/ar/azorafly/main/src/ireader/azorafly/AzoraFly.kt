...
abstract class AzoraFly(private val deps: Dependencies) : SourceFactory(deps = deps) {
     ...
     */
    private suspend fun fetchNovels(page: Int, query: String?): MangasPageInfo {
        return try {
            val perPage = 18
            val url = buildString {
                append("$apiBase/query?page=$page&perPage=$perPage&view=archive")
                append("&seriesType=NOVEL&orderBy=lastChapterAddedAt&orderDirection=desc")
        ...
        return try {
            ...
            while (true) {
                ...
                chapters.forEach { el ->
                    ...
                    val name = ch["title"]?.jsonPrimitive?.contentOrNull
                        ?.takeIf { it.isNotBlank() }
                        ?: if (number > 0) "الفصل $number" else chapterSlug
