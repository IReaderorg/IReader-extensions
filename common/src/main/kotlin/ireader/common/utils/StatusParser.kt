...
object StatusParser {
    ...
    fun addCustomKeywords(status: Long, vararg keywords: String) {
        val targetSet = when (status) {
            MangaInfo.ONGOING -> ongoingKeywords as MutableSet
            MangaInfo.COMPLETED -> completedKeywords as MutableSet
            MangaInfo.ON_HIATUS -> hiatusKeywords as MutableSet
            MangaInfo.CANCELLED -> cancelledKeywords as MutableSet
            else -> return
        }
